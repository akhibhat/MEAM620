...
%     neighbours = getneighbours(map,startidx,allPoints);
    ...
    point = allPoints(startidx).point;
    ...
    j = 1;
    for rows = 1:6
        if ((min(nbours(rows,:)>=lowsub) == 1) && (min(nbours(rows,:)<=upsub == 1))) == 1
    ...
    for i = 1:size(neighbour,1)
        nbouridx(i) = sub2ind(size(map.occgrid),neighbour(i,1),neighbour(i,2),neighbour(i,3));
        if allPoints(nbouridx(i)).visnovisobs ~=0
            neighbour2(j,:) = neighbour(i,:);
            j = j+1;
        end
    end

    neighbours = sub2pos(map,neighbour2);
...
%         nbours = getneighbours(map,nextindex,allPoints);
        point = allPoints(nextindex).point;
        vpoint = pos2sub(map,point);

        nbour(1,:) = vpoint + x_neigh;
        nbour(2,:) = vpoint - x_neigh;
        nbour(3,:) = vpoint + y_neigh;
        nbour(4,:) = vpoint - y_neigh;
        nbour(5,:) = vpoint + z_neigh;
        nbour(6,:) = vpoint - z_neigh;

        j = 1;
        for rows = 1:6
            if ((min(nbour(rows,:)>=lowsub) == 1) && (min(nbour(rows,:)<=upsub == 1))) == 1
               neighbour(j,:) = nbour(rows,:);
               j = j + 1;
            else
                continue;
            end
...
%         notvisitedcost = [allPoints(notvisited).cost2reach];
