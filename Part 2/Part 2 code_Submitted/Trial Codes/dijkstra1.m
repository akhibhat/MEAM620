...
map = load_map('map1.txt', 0.25,1,0);
...
if (allPoints(startidx).visnovisobs == 0) || (allPoints(goalidx).visnovisobs == 0)
    ...
    allPoints(startidx).totalcost = allPoints(startidx).cost2reach + heuristic(astarvar,goal,point);
    ...
    for i = 1:size(nbouridx,1)
        ...
        allPoints(nbouridx(i)).totalcost = allPoints(nbouridx(i)).cost2reach + heuristic(astarvar,goal,allPoints(nbouridx(i)).point);
    ...
    while (nextindex ~= goalidx) && (pathnotfound == 0)
        ...
        for i = 1:size(nextnbouridx,1)
            tcost = allPoints(nextindex).cost2reach + heuristic(astarvar,goal,allPoints(nextnbouridx(i)).point) + 1;
            if tcost < allPoints(nextnbouridx(i)).totalcost
...
%                 allPoints(nextnbouridx(i)).totalcost = allPoints(nextnbouridx(i)).cost2reach + heuristic(astarvar,goal,allPoints(nextnbouridx(i)).point);
%                 updateidx = find(nextindexcost(:,2) == nextnbouridx(i));
%                 nextindexcost(updateidx,1) = allPoints(nextnbouridx(i)).totalcost;
%             else
%                 c2r = allPoints(nextnbouridx(i)).totalcost;
%                 newc2r = allPoints(nextindex).cost2reach + 1 + heuristic(astarvar,goal,allPoints(nextnbouridx(i)).point);
%                 if newc2r < c2r
%                     c2r = newc2r;
%                     allPoints(nextnbouridx(i)).parent = nextindex;
