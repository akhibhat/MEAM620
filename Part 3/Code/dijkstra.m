...
%     nextindexcost(delidx,:) = [];
    ...
    for i = 1:size(nbouridx,2)
        ...
        allPoints(nbouridx(i)).totalcost = allPoints(startidx).totalcost + abs(norm([allPoints(nbouridx(i)).point]-[allPoints(startidx).point])) + heuristic(astarvar,goal,allPoints(nbouridx(i)).point);
...
%         nextindexcost(delidx,:) = [];
        ...
        for i = 1:size(nextnbouridx,2)
            tcost = allPoints(nextindex).totalcost + abs(norm([allPoints(nextnbouridx(i)).point]-[allPoints(nextindex).point])) + heuristic(astarvar,goal,allPoints(nextnbouridx(i)).point);
